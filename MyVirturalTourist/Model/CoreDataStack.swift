...
class DataController {
    ...
    var obContext: NSManagedObjectContext {
    ...
    init?(modelName:String) {
        ...
        self.model = model
               ...
               // create a context and add connect it to the coordinator
               obContext = NSManagedObjectContext(concurrencyType: .mainQueueConcurrencyType)
               obContext.parent = obContext
               
               // Create a background context child of main context
               backgroundContext = NSManagedObjectContext(concurrencyType: .privateQueueConcurrencyType)
               backgroundContext.parent = obContext
    ...
    func saveContext() throws {
        obContext.performAndWait() {
            
            if self.obContext.hasChanges {
                do {
                    try self.obContext.save()
                } catch {
    ...
    func fetchPin(_ predicate: NSPredicate, entityName: String, sorting: NSSortDescriptor? = nil) throws -> Pin? {
        ...
        guard let pin = (try obContext.fetch(fr) as! [Pin]).first else {
            return nil
        }
        return pin
    }
    ...
    func fetchAllPins(_ predicate: NSPredicate? = nil, entityName: String, sorting: NSSortDescriptor? = nil) throws -> [Pin]? {
        ...
        guard let pin = try obContext.fetch(fr) as? [Pin] else {
            return nil
        }
        return pin
    }
